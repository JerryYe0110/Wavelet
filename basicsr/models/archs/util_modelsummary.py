...
# --------------------------------------------
'''
def get_model_profile(model, input_dim):
    if torch.cuda.is_available():
        model = model.cuda()
    with torch.no_grad():
        # activation [M], flops[G], parameters[M]
    ...
    return flops/10**9, activations/10**6, num_parameters/10**6
...
def measure_throughput_gpu(model, input_dim):
    model = model.cuda()
    ave_time = []
    warmup = [torch.randn(input_dim) for i in range(100)]
    Data = [torch.randn(input_dim) for i in range(2000)]
    for x in warmup:
        x = x.cuda()
    ...
    for x in Data:
        ...
        model(x)
        tic2 = int(round(time.time()*1000)) - tic1
        ave_time.append(tic2)
    return statistics.mean(ave_time)/2000
